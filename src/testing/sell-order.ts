 ...
 */
 ...
 async function main(fromPrivateKey: string, tokenAddress: string, tokenId: string, saleAmount: string, bulksale: string ) {
     ...
     let bulksaleint:number =+bulksale
     let tokenIdint:number =+tokenId
     for (let i: number = +tokenId; i < bulksaleint+tokenIdint; i++) 
    {
        const order_result = await sellNFT(sellerClient, sellerClient.address, tokenAddress, i.toString(), saleAmount);
        console.log(i, ":Created sell order with id:", order_result.order_id, order_result.status);
     }
 ...
 const argv = yargs(process.argv.slice(2))
   .usage('Usage: -f <from_private_key> -n <token_address> -i <token_id> -a <sale_amount>  -b <bulk sell order create> ')
   .options({
     f: { describe: 'Seller private key', type: 'string', demandOption: true },
     ...
     b: { describe: 'bulk sell order volume', type: 'string', demandOption: true }
   })
   .parseSync();
         
 main(argv.f, argv.n, argv.i, argv.a, argv.b)
